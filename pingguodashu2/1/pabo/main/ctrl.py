...
def _user_exists(phone):
    '''判断是否已经存在某个分类'''
    values = get_all_users(0, 100000)
    for u in values:
...
def get_all_users(offset=0, limit=10, hasPass=False):
    '''获取所有的用户列表'''
    users = list(get_all_valid_uid(cid))[::-1][offset:]
    for aid in arts:
        user = get_user(aid, False)
        if user is None:
            continue
        yield user
...
def get_all_valid_uid(cid=None):
    '''将文章索引去除0，代表真实的文章索引'''
    arts = kv.get(settings.K_USER) or ''
...
def get_user(uid, hasPass):
    ...
    user = kv.get(settings.K_USER_INFO % uid)
    ret = {'name': user['name'],
           'phone': user['phone'],
        }
    if hasPass == True:
...
def add_article(title, md, cid):
    ...
    kv.set(settings.K_ART_PRICE % aid, _get_price(md))
    kv.set(settings.K_ART_BODY % aid, _get_body(md))
    return ''
...
def get_all_articles(offset=0, limit=10, need_abs=True, need_md=False,
        cid=None):
...
def get_article(aid, need_abs=True, need_md=False):
    ...
    if need_abs:
        ...
        ret['price'] = kv.get(settings.K_ART_PRICE % aid)
        ret['body'] = kv.get(settings.K_ART_BODY % aid)
        # need_html = False
    if need_md:
